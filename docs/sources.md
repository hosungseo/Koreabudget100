...
- 범용 탐색기 기준 스냅샷: `exe_ymd=20260630`
- 단일 기준 사례 시계열: `20260131`, `20260228`, `20260331`, `20260430`, `20260531`, `20260630`, `20260718`
- 원 데이터 안내: https://www.data.go.kr/data/15059434/openapi.do

지자체 단계는 7자리 `laf_cd`에서 `code[2:5] == "000"`이면 광역본청, 그 외는 기초로 분류한다. 중앙 세부사업명뿐 아니라 설명자료에 금액이 명시된 내역사업명도 보수적인 검색 키워드로 쓸 수 있다. 결과는 명칭 변형을 고려한 `keyword_candidate`이며 확정 교부처 교차표가 아니다. `exe_ymd`별 변화는 거래·교부 이력이 아니라 각 조회일의 스냅샷 상태 변화다. 광역·기초 행은 같은 재원을 중복 표현할 수 있다.

관련 정규화 산출물:

- `data/normalized/lofin_local_transfer_candidates_2026.json` — 범용 탐색기 2026-06-30 후보
- `data/normalized/reference_lofin_timeline_2026.json` — `사회연대경제` 7개 조회일 후보 시계열
