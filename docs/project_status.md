...
| LOFIN 조회 가능 중앙사업 / 고유 키워드 | 160 / 140 |
| LOFIN 양수 후보가 발견된 중앙사업 | 63 |
| LOFIN `keyword_candidate` 행 | 2,271 |
...
- LOFIN은 중앙사업 6단계 복합키가 붙은 후보 행만 해당 사업에 연결한다.
...
- 중복 제거한 실제 API 조회 140개
- 양수 후보가 발견된 중앙사업 63개
- 정규화·중복 제거 후보 행 2,271개

각 행에는 `central_business_key`, 중앙사업명, 사용 키워드와 생성 전략, 중앙 지자체 이전액, `match_status=keyword_candidate`를 저장한다. 같은 키워드를 공유하는 중앙사업이 있을 수 있으므로 이 행 수와 반영액 합은 확정 교부처·순지출 총액이 아니다.
...
- LOFIN 2,271행은 모두 비가산 `keyword_candidate` 점선으로 표시
...
bash code/run_pipeline.sh --lofin-cache-only # 기존 140개 캐시로 LOFIN 재생성
```
